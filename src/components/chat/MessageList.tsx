...
function toolLabel(tool: { toolName: string; args?: Record<string, unknown>; state: string }): string {
  const done = tool.state === "result";
  const filename = tool.args?.path
    ? (tool.args.path as string).split("/").pop()!
    : "";

  if (tool.toolName === "str_replace_editor") {
    const cmd = tool.args?.command as string | undefined;
    if (cmd === "create") return done ? `Created ${filename}` : `Creating ${filename}`;
    if (cmd === "str_replace" || cmd === "insert") return done ? `Edited ${filename}` : `Editing ${filename}`;
    if (cmd === "view") return done ? `Read ${filename}` : `Reading ${filename}`;
    if (cmd === "undo_edit") return done ? "Undone edit" : "Undoing edit";
  }

  if (tool.toolName === "file_manager") {
    const cmd = tool.args?.command as string | undefined;
    if (cmd === "rename") return done ? `Renamed ${filename}` : `Renaming ${filename}`;
    if (cmd === "delete") return done ? `Deleted ${filename}` : `Deleting ${filename}`;
  }
                              ...
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs font-mono border border-neutral-200">
                                {tool.state === "result" && tool.result ? (
                                  <>
                                    ...
                                    <span className="text-neutral-700">{toolLabel(tool)}</span>
                                  </>
                                ) : (
                                  <>
